...
class base_action_rule(osv.osv):
    ...
    _columns = {
        ...
        'filter_pre_id': fields.many2one('ir.filters', string='Before Update Filter',
            ...
            deprecated='Will be removed with OpenERP v8; use pre_domain field instead',
            help="If present, this condition must be satisfied before the update of the record."),
        'filter_id': fields.many2one('ir.filters', string='Filter',
            ondelete='restrict',
            domain="[('model_id', '=', model_id.model)]",
            deprecated='Will be removed with OpenERP v8; use domain field instead',
    ...
    def _wrap_create(self, old_create, model):
        ...
        def create(cr, uid, vals, context=None):
            ...
            for action in self.browse(cr, uid, action_ids, context=context):
                domain = action.domain or (action.filter_id and action.filter_id.domain) or False
    ...
    def _wrap_write(self, old_write, model):
        ...
        def write(cr, uid, ids, vals, context=None):
            ...
            for action in actions:
                pre_domain = action.pre_domain or (action.filter_pre_id and action.filter_pre_id.domain) or False
